...
//Serial.println("data='" + data + "'; cmd='" + cmd + "'");
  
  if (!powerOn && cmd != "PWR") {
    NotifyError(F("Питание отключено. Режим не активирован."));  
    return;
  }
  ...
  if (cmd == "BR") {
    ...
    if (cnt == 2) {      
      ...
      NotifyError(String(F("Неверные параметры: ожидается: BR:XXX; получено: ")) + data);      
    }
    return;
  }
  ...
  if (cmd == "PM") {
    ...
    if (cnt == 1) {
      ...
      NotifyError(String(F("Неверный режим: ")) + String(iMode));
    }
      ...
      if (isSpecMode) {
        NotifyInfo(String(F("PM: режим ")) + String(iMode)+ String(F(" - специальный, не имеет настроек.")));
        return;
      }
      ...
      NotifyError(String(F("Неверные параметры: ожидается: PM или PM:N или PM:N:T:D:S:P:U:A; получено: ")) + data);
    }
  ...
  if (cmd == "EDT") {
    ...
    if (cnt == 2) {
      ...
      if (isSpecMode) {    
        NotifyError(String(F("Изменение настроек ")) + String(iMode) + String(F(" не поддерживается")));
      }
      ...
      NotifyError(String(F("Неверные параметры: ожидается: EDT:N; получено: ")) + data);
    }
  ...
  if (cmd == "SV") {
    if (cnt == 1) {
      ...
      NotifyError(String(F("Неверные параметры: ожидается: SV; получено: ")) + data);
    }
    return;
  }
  ...
  if (cmd == "US") {
    ...
    if (cnt == 3) {
      ...
      if (isSpecMode) {
        //NotifyInfo(String(F("Mode: ")) + String(iMode)+ String(F(" is special")));
      }
      ...
      NotifyError(String(F("Неверные параметры: ожидается: US:N:A; получено: ")) + data);
    }
    return;
  }
  ...
  if (cmd == "DO") {
    ...
    if (!isSpecMode && (iMode < 2 || iMode > MAX_EFFECT)) {    
      ...
      NotifyError(String(F("Неверные параметры: ожидается: DO:N; получено: ")) + data);
  ...
  if (cmd == "RGB") {    
    ...
    if (cnt == 4) {
      ...
      NotifyError(String(F("Неверные параметры: ожидается: RGB:DDD или RGB:#XXXXXX или RGB:R:G:B; Получено: ")) + data);
      return;
    }
  ...
  if (cmd == "PWR") {
    ...
    NotifyError(String(F("Неверные параметры: ожидается: PWR или PWR:ON или PWR:OFF; получено: ")) + data);
    return;
  }
  ...
  if (cmd == "RND") {
    ...
    if (cnt == 1) {
      ...
      if (randomModeOn)
        Serial.println(F("Автосмена режимов: включено"));
      else
    ...
    NotifyError(String(F("Неверные параметры: ожидается: RND; получено: ")) + data);
    return;
  }
  ...
  NotifyError(String(F("Нераспознано -> ")) + data);
}
